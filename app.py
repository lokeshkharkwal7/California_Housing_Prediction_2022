...
from housing_project.pipeline.main_pipeline import Pipeline
...
try:
    model_regression = pickle.load(open('Final_Pipeline\\final_pipeline.pkl', 'rb'))
...
def predict_regressor():
        ...
        output=output_generation(df)
        op=output.generate_output()
          
        op=round(op,2)
        print(op)
         
        prediction_result = f"Predicted Result: {op}"
